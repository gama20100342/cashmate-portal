<?php	
	...
	echo '<div class="row"><div class="col-md-6"><div class="box box-default color-palette-box">';
						...
						switch($form){
							...
							default: 
								switch($index){
									...
									case "bday":
										echo '<td><input type="date" name="data['.$form.']['.$index.']" class="form-control input-sm" value="'.$this->data[$form][$index].'"></td></tr>';	
									break;
									
									default:
										echo '<td>' . $this->Form->input($index, array('label' => false, 'empty' => true, 'class' => 'form-control input-sm', 'id' => $index, 'autocomplete' => 'off')) . '</td></tr>';			
									break;
								}
							break;
						}	
		...
		echo '<div class="box-footer clearfix">';
			echo $this->Form->submit($endbtn, array('class' => 'btn btn-primary', 'title' => 'Submit all information'));
		echo '</div>';
	echo '</div></div></div>';
?>
