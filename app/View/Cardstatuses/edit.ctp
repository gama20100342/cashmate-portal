...
<?php echo $this->Form->create('Cardstatus', array('class' => 'data-form', 'id' => 'new_cardstatus_form')); ?>
	...
	<?php
		...
		echo $this->App->showForminputs(array(				
				...
				array('input' => 'status_id', 'label' => 'Status', 'empty' => false, 'default'  => $this->request->data["Cardstatus"]["color"], 'type' => 'select', 'options' => $statuses)
				));
	...
	<div class="col-md-6 nopadding-right input-wrap">
		<?php echo $this->Form->input('color', array('label' =>'Select Color', 'id' => 'showPaletteOnly', 'class' => 'noradius noshadow border-bottom text-left form-control comonkeypress input-md '));  ?>
	</div>
	
	<div class="clear"></div>
	</fieldset>
	<?php echo $this->App->formEnd('SAVE CHANGES', '#new_cardstatus_form'); ?>
</div>
<?php
	...
	echo $this->Js->Buffer('
		$(document).ready( function(){
			$("#showPaletteOnly").spectrum({
				...
				showPalette:true,
				color: "'.$this->request->data["Cardstatus"]["color"].'",
				palette: [
